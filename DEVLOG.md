...
**Function Transformations** (`/advanced-math/function-transformations`): Interactive grapher shows the base curve y = x² (faint dashed ghost) and a live transformed curve controlled by four sliders/controls: h (horizontal shift, inside the function), k (vertical shift, outside), a (vertical scale, outside), and a reflection checkbox. Positive h in the slider = f(x − h) = shifts RIGHT — the UI makes the sign flip explicit. The accent dot marks the transformed vertex; the ghost dot stays fixed at the origin so the shift is visible. Sections: The Six Transformations (two-column table: outside/vertical vs. inside/horizontal), How it shows up on the SAT (words-to-equation and table-evaluation disguises), worked example (f(x) = x² → f(x − 3) + 2, vertex moves (0,0) → (3,2), sign trap called out explicitly).

**Polynomials** (`/advanced-math/polynomials`): Three toggleable factors — (x+2) at x=−2 mult 1 (crosses), (x−1)² at x=1 mult 2 (bounces), (x−3) at x=3 mult 1 (crosses) — plus a leading-coefficient sign toggle. The factored form string, degree, roots-with-cross/bounce badges, and four active/inactive end-behavior rules all update live. Default all-on: (x+2)(x−1)²(x−3), degree 4, positive leading, both ends up, showing all three root behaviors at once. Sections: Core Reading Rules (root = opposite sign; odd mult → crosses, even → bounces; degree = sum of mults), How it shows up on the SAT (match-to-graph and distinct-zeros-count disguises), worked example: f(x) = (x−1)²(x+2) — bounces at 1, crosses at −2, degree 3 odd positive, ↓ left / ↑ right.

**Exponential Functions in Context** (extends existing `/advanced-math/exponential-functions`): New "Exponential Functions in Context" section appended after the existing worked example. Covers the growth/decay templates A = P(1 ± r)ᵗ (linked explicitly to y = a·bˣ), compound interest A = P(1 + r/n)^(nt) with the monthly-vs-annual comparison, half-life formula A = A₀(½)^(t/h), the percent-to-base trap, and a short worked example ($2,000 at 3% → A = 2000(1.03)ᵗ, ~$2,688 after 10 years). Build clean at 46 modules.
